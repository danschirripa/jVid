package com.javashell.control.protocols;
...
public class VISCA {
	...
	public static byte[] zoomCommand(int direction, byte speed) {
		...
		case PTZ_IN:
			...
			speed = (byte) Integer.parseInt("2" + Integer.toHexString(speed), 16);
		...
		case PTZ_OUT:
			...
			speed = (byte) Integer.parseInt("3" + Integer.toHexString(speed), 16);
	...
	public static byte[] directZoomCommand(byte directZoom) {
		byte[] command = new byte[9];
		command[0] = (byte) 0x81;
		command[1] = (byte) 0x01;
		command[2] = (byte) 0x04;
		...
		String zoomHexString = Integer.toHexString(directZoom);
		char[] hexBytePlaces = zoomHexString.toCharArray();

		int lengthDelta = 0;
		for (int i = 4; i < 4 + 4; i++) {
			if (i >= hexBytePlaces.length) {
				command[i] = 0;
				continue;
			}
			command[i] = (byte) Integer.parseInt("0" + hexBytePlaces[i - 4]);
		}
		command[8] = (byte) 0xFF;
		return command;
	}
