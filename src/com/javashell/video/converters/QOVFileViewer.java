...
public class QOVFileViewer {
	private File inputFile;
	private final QOIStreamIngestorC qoi;
	private BufferedImage bufFrame;

	public QOVFileViewer(File inputFile) {
		this.inputFile = inputFile;
		qoi = new QOIStreamIngestorC();
	}
			...
			public void run() {
				try {
					while (fin.available() > 4) {
						...
						final byte[] decodedImage = qoi.convertToBytes(encoded, width, height, subSegments);
