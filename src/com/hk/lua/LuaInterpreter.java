...
/**
 * This utility class encapsulates an entire Lua environment within a single
 * Java class/package. This class is instantiated using the {@link Lua} class,
 * which allows you to construct a Lua interpreter using a
 * {@link java.io.Reader}. The reader is fully read and interpreted as Lua code.
 * Which can then be executed in a myriad of fashions. There can be various
 * types of variables and objects that can be directly tied to Java methods,
 * fields, or code.
 *
 * @see Lua#interpreter()
 * @see Lua#reader
 ...
 * @see <a href=
 *      "https://www.lua.org/manual/5.3/">https://www.lua.org/manual/5.3/</a>
 *
 * @author theKayani
 */
public class LuaInterpreter implements Tokens {
	 ...
	 *
	 * <p>
	 * Extra data it data that is tied to this specific interpreter. Completely
	 * optional and has no effect on execution of Lua. Just for utility and data
	 * consistency sake.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Get the extra data under a certain key. The data returned should be able to
	 * be cast to the <code>cls</code> specified. If so, it is boxed and returned.
	 * </p>
	 *
	 * <p>
	 * Extra data it data that is tied to this specific interpreter. Completely
	 ...
	 *
	 * @param key a {@link java.lang.String} to match to a key
	 * @param cls a {@link java.lang.Class} to cast the returned value to
	 * @param <T> a T class
	...
	/**
	 * <p>
	 * Get the extra data under a certain key, or a default if there data found
	 * under the key was null or not found.
	 * </p>
	 *
	 * <p>
	 * Extra data it data that is tied to this specific interpreter. Completely
	 ...
	 * @param key a {@link java.lang.String} to match to a key
	 * @param cls a {@link java.lang.Class} to cast the returned value to
	 * @param def a T object or the default if there was one provided
	 * @param <T> a T class
	 * @return a T object, or the def value if the data was null or there was no
	 *         found data.
	 */
	...
	/**
	 * <p>
	 * Get the extra data under a certain key, cast as a {@link LuaObject}. This is
	 * useful because it will not return null, it will instead return
	 * {@link Lua#NIL} if there was no valid data found.
	 * </p>
	 *
	 * <p>
	 * Extra data it data that is tied to this specific interpreter. Completely
	 ...
	 *
	 * @param key a {@link java.lang.String} to match to a key
	 * @return a {@link com.hk.lua.LuaObject} object, or {@link Lua#NIL} if there
	 *         was no valid data found under the specified key.
	 */
	...
	/**
	 * <p>
	 * Get the extra data under a certain key, cast as a {@link String}. This will
	 * return null if there was no valid data found.
	 * </p>
	 *
	 * <p>
	 * Extra data it data that is tied to this specific interpreter. Completely
	 ...
	 *
	 * @param key a {@link java.lang.String} to match to a key
	 * @return a {@link java.lang.String} object, or null if there was no valid data
	 *         found under the specified key.
	 */
	...
	/**
	 * <p>
	 * Set the data under a certain key. This value can then later be retrieved
	 * using the {@link #getExtra} methods. These have various overloaded helper
	 * functions to return the value cast as another type.
	 * </p>
	 *
	 * <p>
	 * Extra data it data that is tied to this specific interpreter. Completely
	...
	/**
	 * <p>
	 * Remove a certain key from the extra data. This will result in the default
	 * value being used (if passed) or a null value when using the {@link #getExtra}
	 * methods.
	 * </p>
	 *
	 * <p>
	...
	/**
	 * <p>
	 * Get the global Lua environment with global variables and functions.
	 * Conventionally, the developer would use this method to set the various global
	 * variables for this specific environment before calling the {@link #execute}
	 * method, or calling a specific method by name.
	 * </p>
	 *
	 * <p>
	 * Calling this before <code>execute</code> will only contain the library
	 * imported methods and data. After calling the <code>execute</code> method,
	 * will the environment be populated by the variables from the Lua code
	 * interpreted.
	 * </p>
	 *
	...
	/**
	 * Immediately execute a string of Lua code using this global environment under
	 * a new Lua chunk.
	 *
	 * @param cs the Lua code to interpret
	 * @return the result from this execution. If this code had a return statement.
	 */
	...
	/**
	 * Immediately execute a reader of Lua code using this global environment under
	 * a new Lua chunk.
	 *
	 * @param reader a {@link java.io.Reader} to provide the Lua code to interpret
	 * @return the result from this execution. If this code had a return statement.
	 */
	...
	/**
	 * Immediately execute a reader of Lua code using this global environment under
	 * a new Lua chunk. This function executes the Lua code under a certain module,
	 * the result is then stored in the case that this function is called again with
	 * the same module. If so, the reader is ignored and the saved value is returned
	 * instead.
	 *
	...
	/**
	 * <p>
	 * Import the individual elements of a Lua Library directly into the global
	 * environment. Whether the elements are placed in the global space, or under a
	 * certain table is defined by the constructor when instantiating the library
	 * object.
	 * </p>
	 *
	 * <p>
	 * To use any of the default libraries, you don't need to define any functions
	 * yourself. All you need to do is import the various standard libraries under
	 * {@link LuaLibrary}.
	 * </p>
	 *
	 ...
	 * {@link #compile} was not called first, the code is first interpreted and
	 * compiled before executed.
	 * </p>
	 *
	 * <p>
	 * The <code>args</code> parameter is for providing a variable amount of
	 * arguments to the Lua code. According to
	 * <a href="https://www.lua.org/manual/5.3/manual.html#3.4.11">The Lua 5.3
	 * Manual</a>, the provided arguments are converted into their LuaObject
	 * equivalents and stored in the <code>...</code> varargs variable in the Lua
	 * code.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Attempt to fully read the provided {@link java.io.Reader} and interpret it as
	 * Lua code. If there are any Lua syntax errors, this function will throw a
	 * {@link LuaException}.
	 * </p>
	 *
	 * <p>
	 * If not, the code is saved to be executed using the {@link #execute} command.
	 * If this is not called beforehand, it is called by default by the
	...
	private static boolean readStatement(Tokenizer tkz, String source, List<LuaStatement> lst, int flags)
			throws IOException {
		boolean res = false;
		...
		case T_LOCAL:
			if (tkz.next()) {
				...
				case T_FUNCTION:
					...
					res = true;
					lst.add(new LuaAssignStatement(line, new LuaVariable[] { var },
							new LuaExpressions(readFunction(tkz, source, false, name))));
					break;
				default:
					throw unexpected(tkz, source, "name");
				}
	...
	private static LuaLocation readLocation(Tokenizer tkz, String source, boolean wm) throws IOException {
		...
		tklbl: while (tkz.next()) {
			...
			case T_STRING:
				if (wm) {
					line = tkz.line();
					LuaExpressions exps = new LuaExpressions(
							new LuaExpression(source, new LuaString((String) tkz.value())));
	...
	private static LuaExpression readExpression(Tokenizer tkz, String source, boolean strict) throws IOException {
		...
		tklbl: while (tkz.next()) {
			...
			if (hasValue) {
				...
				case T_FLR_DIVIDE:
				case T_MODULO:
					while (!ops.isEmpty() && (Tokenizer.prec(type) < Tokenizer.prec(ops.peek())
							|| !ra && Tokenizer.prec(type) == Tokenizer.prec(ops.peek()))) {
						res.add(ops.pop());
						res.add(ops.pop());
					}
				...
				case T_NOT:
					while (!ops.isEmpty()) {
						...
						if (Tokenizer.prec(type) < Tokenizer.prec(n)
								|| !ra && Tokenizer.prec(type) == Tokenizer.prec(n)) {
							res.add(ops.pop());
							res.add(ops.pop());
						} else
							break;
	...
	private static LuaFunctionExpression readFunction(Tokenizer tkz, String source, boolean self, String name)
			throws IOException {
